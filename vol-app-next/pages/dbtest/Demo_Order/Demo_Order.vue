...
<template>
	<vol-view ref="viewRef" :table="table" :columns="columns" :table-data="tableData"
		...
		<!--表格配置 -->
		<vol-table :default-load="true" ref="tableRef" :ck="false" :index="false" :url="tableUrl"
			@rowClick="modelOpenBefore" :loadBefore="searchBefore" :loadAfter="searchAfter" :direction="direction"
			:titleField="table.titleField" :columns="columns" :table-data="tableData">
		</vol-table>
	</vol-view>
</template>
<script setup>
	...
	//设置默认查询条件
	// nextTick(()=>{
	// 	searchFormFields.OrderNo = "P202412090004";
	// 	loadData()
	// })
