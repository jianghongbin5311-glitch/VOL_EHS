...
<template>
	<vol-edit ref="editRef" :id="id" :table="table" :detail="detail" :details="details" :editFormFields="editFormFields"
		...
		:delRowAfter="delRowAfter" :loadFormAfter="loadFormAfter" :detailFormInited="detailFormInited" @detailExtraClick="detailExtraClick">
		<template #header>
			<view style="margin-bottom: 22rpx;">
				<vol-alert>1.移动端支持一对多零代码自动生成,并支持扩展
...
<script setup>
	import options from "./Demo_ProductOptions.js";
	import {
		onLoad
	...
	//明细表添加选择按钮
	const detailFormInited=(tableInfo)=>{
		  if (tableInfo.table=='Demo_ProductColor') {
		    const option=	tableInfo.edit.formOptions.find(x=>{return x.field=='Color'});
			option.extra={
				button:true,
				type:'primary',
				icon:"search",
		...
		//明细表添加选择按钮事件
	const detailExtraClick=(tableInfo,item,detailFields)=>{
		proxy.$toast('点击了按钮')
		   //缓存方法，另一个页面选择时回调此方法,proxy.base.getItem('productColorSelect')(row);
			proxy.base.setItem('productColorSelect',(row)=>{
				 // detailFields.字段=row.字段
			})
	}
