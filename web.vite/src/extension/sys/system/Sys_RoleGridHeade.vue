            ...
            <div v-show="activeName == 'data' && !data.children">
              <el-radio-group v-model="data.authMenuData">
                ...
                <el-badge
                  v-if="!data.children"
                  class="item"
                  :is-dot="(data.authMenuData || '').includes(',') ? 1 : 0"
                  @click="showCustom(data)"
                >
                  <el-tag link>+{{ $ts('自定义') }}</el-tag>
                </el-badge>
              </el-radio-group>
            </div>
            <div v-show="activeName == 'field' && !data.children" @click="openAuthFields(data)">
              <a><span class="el-icon-edit-outline"></span>{{ $ts('字段权限') }}</a>
              <!-- <el-badge
                class="item"
                @click="openAuthFields(data)"
              >
                <el-tag link>+{{ $ts('字段权限') }}</el-tag>
              </el-badge> -->
            </div>
          </div>
        </template>
