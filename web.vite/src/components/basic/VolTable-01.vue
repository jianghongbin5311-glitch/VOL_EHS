  ...
  >
    ...
    <template v-if="!paginationHide">
      <div class="block pagination" key="pagination-01">
        <div><slot name="pagination"></slot></div>
...
const getTableData = () => {
  return props.url ? rowData : props.tableData;
};
const getTable = () => {
  return table.value;
};
...
const getCellClass = ({ row, column, rowIndex, columnIndex }) => {
  const b = props.columns.some((x) => {
    return (
      x.field === column.property && x.edit && (x.edit.keep || edit.rowIndex === rowIndex)
    );
  });
...
const rowClick = (row, column, event) => {
  tableRowClick(proxy, props, edit, nextTick, emit, row, column, event);
};

//图片预览
const viewer = ref(null);
const viewImg = (row, column, $event, index) => {
...
const load = async (query, isResetPage) => {
  ...
  //重置合计
  loadDataSummaries(props, data, summaryData);
...
const selectionChange = (selection) => {
  selectionRowChange(props, emit, selection, table.value);
};
...
//添加行
const addRow = (row) => {
  addTableRow(proxy, props, getTable(), row);
};
...
const handleTableClickOutside = (event) => {
  if (refTable.value?.contains(event.target)) {
    ...
    if (edit.rowIndex != -1) {
      let row = getTableData[this.edit.rowIndex];
      rowEndEdit(row);
      edit.rowIndex = -1;
    }
  }
...
onMounted(() => {
  ...
  initSortable(props, emit, nextTick, refTable.value, getTableData());
});
onUnmounted(() => {
  if (hasEdit()) {
