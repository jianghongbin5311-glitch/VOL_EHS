    ...
    >
      <template #reference>
        <el-input
          ...
          @keypress="onKeyPress"
          @input="onInput"
          :placeholder="$ts(placeholder || '请输入')"
          :size="size"
          :clearable="false"
      ...
      >
        <div class="input-search" v-if="searchForm.length || !single">
          ...
          <el-button @click="searchClick" type="primary" v-if="searchForm.length" plain
...
export default {
  ...
  setup(props, context) {
    ...
    const inputClick = () => {
      if (!props.url) {
        proxy.$message.error(
          "未配置url属性,见文档:http://doc.volcore.xyz/example/selectTable.html"
        );
        return;
      }
      currentIndex = -1;
