  ...
  >
    ...
    <div class="view-column view-column-title">
      ...
      <div class="view-column-right">
        <el-tooltip class="box-item" effect="dark" placement="top">
          <i class="el-icon-warning-outline"> {{ $ts("固定字段") }}</i>
          <template #content> {{ $ts("设置表头字段固定到左边或者右边显示") }} </template>
        </el-tooltip>
      </div>
      <div class="view-column-right">
        <el-tooltip class="box-item" effect="dark" placement="top">
          <i class="el-icon-warning-outline"> {{ $ts("文本显示") }}</i>
          <template #content>
            {{ $ts("设置表格文本显示位置：靠左、中、右显示") }}
          </template>
        </el-tooltip>
      </div>
    </div>
      ...
      >
        <transition-group>
          <div class="view-column" v-for="(column, index) in columns" :key="index">
            ...
            <div class="view-column-right">
              <el-radio-group v-model="column.fx">
                <el-radio :label="$ts('无')" value="none"> </el-radio>
                <el-radio :label="$ts('左')" value="left"> </el-radio>
                <el-radio :label="$ts('右')" value="right"> </el-radio>
              </el-radio-group>
            </div>
            <div class="view-column-right">
              <el-radio-group v-model="column.a">
                <el-radio :label="$ts('左')" value="left"> </el-radio>
                <el-radio :label="$ts('中')" value="center"> </el-radio>
                <el-radio :label="$ts('右')" value="right"> </el-radio>
              </el-radio-group>
            </div>
          </div>
    ...
    <template #footer>
      <div class="dialog-footer">
        ...
        <el-button color="#626aef" icon="Sort" plain @click="reset">{{
          $ts("重置")
        }}</el-button>
        <el-button type="primary" icon="Check" plain @click="save">{{
          $ts("保存")
        }}</el-button>
      </div>
    </template>
