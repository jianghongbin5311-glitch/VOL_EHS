...
let methods = {
    ...
    if (this.newTabEdit) {
      ...
      this.$tabs.open({
        text:
          ...
          '(' +
          (!this.isCopyClick && value ? this.$ts('编辑') : this.$ts('新建')) +
          ')',
  ...
  initBoxHeightWidth() {
    ...
    this.initExtraHeight() //初始化表格高度，减去自定义过滤高度与查询条件高度
  },
  rowOnChange(row) {
    this.rowChange(row)
  },
  ...
  async copyData(rows) {
    ...
    if (!rows || rows.length == 0) return this.$error(this.$ts('请选择要复制的行'))
    if (rows.length > 1) return this.$error(this.$ts('只能选择一行数据复制'))
