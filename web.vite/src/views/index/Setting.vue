    ...
    <div class="theme-selector">
        ...
        <div>
            <el-radio-group @change="layoutChange" v-model="layoutValue">
                ...
                <el-radio value="left">{{ $ts('侧边导航') }}</el-radio>
            </el-radio-group>
        </div>
    </div>
</template>
...
export default defineComponent({
    ...
    setup() {
        const colors = reactive([
            { name: 'white', color: '#fff', dark: false },
            { name: 'dark', color: '#272929', dark: false },
            { name: 'blue', color: 'rgb(45, 140, 240)', dark: false },
            { name: 'red', color: 'rgb(237, 64, 20)' },
            { name: 'orange', color: '#ff9900' },
            { name: 'gradient_orange', color: 'linear-gradient(to bottom, rgb(220,160,126),  rgb(250,230,170))' },
        ...
        const cacheTheme = () => {
            //侧边双层导航
            if (layoutValue.value == 'left'&&themeValue!='white') {
                themeValue = _t + '-aside';
            } else {
                themeValue = _t;
            }
